...
jest.mock('./useLinkedInApi');
let mockLinkedInApi: ReturnType<typeof useLinkedInApi>;
...
function expectQuoteRequested() {
  expect(mockLinkedInApi.quote).toHaveBeenCalledWith({});
}
...
function expectAuthUrlCalled() {
  expect(mockLinkedInApi.authUrl).toHaveBeenCalledWith({});
}
...
function expectConfirmCalledWith(routeParams: {
  ...
  expect(mockLinkedInApi.confirm).toHaveBeenCalledWith(routeParams);
}
...
function expectAttestationRequested() {
  expect(mockLinkedInApi.requestAttestation).toHaveBeenCalledWith({
    message: { signed: 'quote' },
  });
}

function expectStartOver() {
  expect(mockLinkedInApi.authUrl).toHaveBeenCalled();
}
...
describe('LinkedIn', () => {
  beforeEach(() => {
    ...
    jest.mocked(useLinkedInApi).mockReturnValue(mockLinkedInApi);
  ...
  it('should go through the happy path until redirected to LinkedIn', async () => {
    render(<LinkedIn session={sessionMock} />);

    expectAuthUrlCalled();

    await act(async () => {
      authUrlPromise.resolve('https://linkedIn-auth-url.example');
    });

    await signInWithLinkedIn();
    expect(
      await screen.findByText('Sign in with LinkedIn'),
    ).toBeInTheDocument();
  });
  ...
  it('should show an error when authUrl fails', async () => {
    ...
    expect(mockLinkedInApi.authUrl).toHaveBeenCalledTimes(1);
    await tryAgain();
    expect(mockLinkedInApi.authUrl).toHaveBeenCalledTimes(2);
  });

  it('should finish the happy path after authorization', async () => {
    const { container } = render(
      <MemoryRouter
        initialEntries={[generatePath(paths.linkedInAuth, { secret, code })]}
      >
        <LinkedIn session={sessionMock} />
      </MemoryRouter>,
    );
  ...
  it('should show authorization error', async () => {
    ...
    expect(
      await screen.findByText(
        'There was an error authorizing your LinkedIn account.',
      ),
    ).toBeInTheDocument();
  ...
  it('should advice when the popup is closed', async () => {
    ...
    await continueInWallet();
    expect(mockLinkedInApi.quote).toHaveBeenCalledTimes(2);
  });
