...
async function handler(
  ...
  logger.debug('Dotsama attestation started');

  const session = getSession(request.payload as PayloadWithSession);

  try {
    const response = await getAttestationMessage(session);
    logger.debug('Dotsama attestation completed');
...
export const attestationDotsama: ServerRoute = {
  method: 'POST',
  path: paths.dotsama.attest,
  handler,
  options: {
    validate: {
